package com.koala.module.app.manage.buyer.action;
...
public class AppBuyerOrderAction {
	...
	public void buyer_order_cofirm(HttpServletRequest request,
		...
		OrderForm obj = this.orderFormService.getObjById(CommUtil
				.null2Long(order_id));
		if (obj != null
				&& obj.getUser_id().equals(CommUtil.null2String(user.getId()))) {
						...
						if (obj.getPayment().getMark().equals("payafter")) {// 如果买家支付方式为货到付款，买家确认收货，子订单商品销量增加
							...
							for (Map child_map : goods_map) {
								...
								Map<String, Integer> logspecmap = (Map<String, Integer>) Json
										.fromJson(todayGoodsLog
												.getGoods_sale_info());
								String spectype = child_map
										.get("goods_gsp_val").toString();
								if (logspecmap.containsKey(spectype)) {
									...
									logspecmap.put(spectype, CommUtil
											.null2Int(child_map
													.get("goods_count")));
								}
