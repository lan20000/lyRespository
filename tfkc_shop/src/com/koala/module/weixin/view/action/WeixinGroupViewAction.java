package com.koala.module.weixin.view.action;
...
import com.koala.foundation.domain.GroupArea;
import com.koala.foundation.domain.GroupClass;
import com.koala.foundation.domain.GroupGoods;
import com.koala.foundation.domain.GroupLifeGoods;
import com.koala.foundation.domain.GroupPriceRange;
...
import com.koala.foundation.service.IGroupAreaService;
import com.koala.foundation.service.IGroupClassService;
import com.koala.foundation.service.IGroupGoodsService;
import com.koala.foundation.service.IGroupLifeGoodsService;
import com.koala.foundation.service.IGroupPriceRangeService;
...
import com.koala.view.web.tools.GoodsClassViewTools;
import com.koala.view.web.tools.GroupViewTools;
import com.koala.view.web.tools.NavViewTools;
...
public class WeixinGroupViewAction {
	...
	public ModelAndView group(HttpServletRequest request,
			...
			String type) {
		ModelAndView mv = new JModelAndView("wap/group.html",
		...
		if ("life".equals(type)) {
			mv = new JModelAndView("wap/group_life.html",
					configService.getSysConfig(),
		...
		if (ga_id != null && !ga_id.equals("")) {
			mv.addObject("ga",
					this.groupAreaService.getObjById(CommUtil.null2Long(ga_id))
							.getGa_name());
		} else {
			mv.addObject("ga", "全国");
		}
		if(CommUtil.null2String(orderType).equals("desc")){
			mv.addObject("c_orderType", "asc");
		}else if(CommUtil.null2String(orderType).equals("asc")){
	...
	public ModelAndView group_data(HttpServletRequest request,
			...
			String orderType, String gc_id, String gpr_id, String ga_id,
			String type) {
		ModelAndView mv = new JModelAndView("wap/group_data.html",
				configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		if ("".equals(type) || "goods".equals(type) || type == null) {
			Map params = new HashMap();
		...
		if ("life".equals(type)) {
			mv = new JModelAndView("wap/group_life_data.html",
					configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request,
					response);
			Map params = new HashMap();
	...
	public ModelAndView group_area(HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView("wap/group_area.html",
				configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
	...
	public ModelAndView group_view(HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView("wap/group_view.html",
				configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
	...
	public ModelAndView grouplife_view(HttpServletRequest request,
			HttpServletResponse response, String id) {
		ModelAndView mv = new JModelAndView("wap/grouplife_view.html",
				configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
	...
	public ModelAndView group_evaluation(HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView("wap/group_evaluate.html",
				configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		...
		CommUtil.saveIPageList2ModelAndView(CommUtil.getURL(request)
				+ "/wap/group_evaluation.htm", "", "", pList, mv);
	...
	public ModelAndView group_order(HttpServletRequest request,
			HttpServletResponse response, String id, String goods_id,
			String currentPage) {
		ModelAndView mv = new JModelAndView("wap/group_order.html",
				configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		EvaluateQueryObject qo = new EvaluateQueryObject(currentPage, mv,
				"addTime", "desc");
	...
	public ModelAndView life_order(HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView("wap/life_order.html",
				configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
	...
	public ModelAndView life_order_save(HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView("wap/life_order_pay.html",
				configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		User user = this.userService.getObjById(SecurityUserHolder
				.getCurrentUser().getId());
