...
import com.koala.manage.buyer.tools.CartTools;
import com.koala.manage.delivery.tools.DeliveryAddressTools;
import com.koala.manage.seller.tools.CombinTools;
import com.koala.manage.seller.tools.TransportTools;
import com.koala.module.weixin.view.tools.OrderPayTools;
...
public class WeixinCartViewAction {
				...
				if (inventory_very) {
					...
					for (int i = 0; i < store_ids.length; i++) {// 根据店铺id，保存多个子订单
						...
						if(!StringUtils.isEmpty(integral) && !"0".equals(integral)){
							of.setOrder_integral(BigDecimal.valueOf(CommUtil.null2Double(integral)));//订单结算中使用的积分
						}else{
						...
						if (flag) {
							...
							if (groupFlag) {
								...
								/*if (null != chgid && !"".equals(chgid)) {
									...
									String sql = "update tfshop_group_joiner t set t.joiner_count=t.joiner_count+1" +
					...
					if("0.00".equals(CommUtil.formatMoney(all_of_price)) || 
						...
						main_order.setPayTime(new Date());//支付完成时间 
						if (groupFlag) {
							main_order.setOrder_status(66);//订单状态改为已付款待发货
						} else {
						...
						if (groupFlag) {
							...
							if (null != gj) {
								...
								if (null != chgid && !"".equals(chgid)) {
									...
									String sqls = "update tfshop_group_joiner a set a.joiner_count=a.joiner_count+1 where " +
	 ...
	 * @return
	 */
	/* @SecurityMapping(title = "查看购物车", value = "/wap/goods_group_cart.htm*",
	 rtype = "buyer", rname = "购物流程0", rcode = "wap_goods_cart", rgroup =
	 "移动端购物")*/
	 ...
	 * @return
	 */
	@SecurityMapping(title = "旅游预订", value = "/wap/goods_tour_order.htm*", rtype = "buyer", rname = "旅游预订", rcode = "goods_tour_order", rgroup = "在线购物")
