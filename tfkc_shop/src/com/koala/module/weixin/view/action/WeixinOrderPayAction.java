...
import com.koala.foundation.domain.wechat.WeixinTemplate;
import com.koala.foundation.domain.wechat.WeixinTemplateParam;
...
//			
		...
		if (order.getOrder_cat() == 3) {
			String sqls = "SELECT a.gg_goods_id from tfshop_group_goods a " +
					"where a.id in ( " +
					"SELECT b.rela_group_goods_id from tfshop_group_joiner b " + 
		...
		if("SUCCESS".equalsIgnoreCase(result_code)){//业务结果为success 处理订单信息
			...
			if (main_order != null
				...
				try {
					...
					//更新对应的团购状态
					String sqls = "update tfshop_group_joiner a set a.joiner_count=a.joiner_count+1 where a.child_group_id in ( " +
							"select b.child_group_id from " +
							"(select t.child_group_id from tfshop_group_joiner t where t.rela_order_form_id=:orderId) b)";
					pmap.clear();
					pmap.put("orderId", moId);
					this.queryService.executeNativeSQL(sqls, pmap);
					
					sqls = "update tfshop_group_joiner t set t.status='1'" +
					...
					if (main_order.getOrder_cat() == 3) {  //订单类型为团购
						...
						sqls = "SELECT a.id from tfshop_group_joiner a, " +
								"(SELECT t1.child_group_id from tfshop_group_joiner t1, tfshop_orderform t2 where " +
