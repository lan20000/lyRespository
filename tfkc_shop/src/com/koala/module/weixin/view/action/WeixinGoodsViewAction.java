				...
				if (obj.getGoods_type() == 0) {// 平台自营商品
					...
					/*if (obj.getGroup() != null && obj.getGroup_buy() == 2) {*/// 如果是团购商品，检查团购是否过期
					  if (groupFlag) {	
						...
						String qSql = "SELECT count(1) from tfshop_group_joiner a, tfshop_group_goods b " +
										"where " +
						...
						if (jGFlag.equals("1")) {   //已经参加团购，查询当前用户参加团的信息：参加人昵称、图像、团剩余时间、剩余人数
							...
							String nql = "SELECT t.rela_group_goods_id, t.child_group_id, t1.trueName, t1.headImgUrl, (t2.gg_group_count-t.joiner_count), t.is_group_creator, " +
									" (UNIX_TIMESTAMP(date_sub(t.create_time,interval - t2.gg_max_count HOUR)) - UNIX_TIMESTAMP(now())), DATE_ADD(t.create_time, INTERVAL t2.gg_max_count HOUR) from tfshop_group_joiner t, " +
									" tfshop_user t1, tfshop_group_goods t2 where t.user_id=t1.id and t.rela_group_goods_id=t2.id " +
									...
									" SELECT a.child_group_id from tfshop_group_joiner a where a.user_id=:userId and a.rela_group_goods_id=:gid) " +
							...
							String sql = "SELECT t.rela_group_goods_id, t.child_group_id, t1.trueName, t1.headImgUrl, (t2.gg_group_count-t.joiner_count), t.is_group_creator, " +
									" (UNIX_TIMESTAMP(date_sub(t.create_time,interval - t2.gg_max_count HOUR)) - UNIX_TIMESTAMP(now())), DATE_ADD(t.create_time, INTERVAL t2.gg_max_count HOUR) from tfshop_group_joiner t, tfshop_user t1, tfshop_group_goods t2 where" +
...
//				"and obj.status=1 and date_sub()", params, -1, -1);
		...
		List list = this.queryService.nativeQuery("SELECT t.rela_group_goods_id, t.child_group_id, t1.trueName, t1.headImgUrl, (5-t.joiner_count)," +
				" (UNIX_TIMESTAMP(date_sub(t.create_time,interval -24 HOUR)) - UNIX_TIMESTAMP(now())) from tfshop_group_joiner t, tfshop_user t1 where" +
				" t.user_id=t1.id" +
				" and t.user_id!=:userId" +
				" and t.rela_group_goods_id=:gid and t.status='1'" +
				" and is_group_creator = '1'" +
