...
public class GoodsViewTools {
	...
	public GoodsLog getTodayGoodsLog(long id) {
		try {
			...
			if (goodsLogList.size() == 0) {
				...
				System.out.println(good.getGc().getId());
	...
	//根据栏目id查询栏目下的推荐商品
	public Goods queryRecommedByGc(String gcId){
		Map<String ,Object> map = new HashMap<String ,Object>();
		map.put("gcId",  Long.parseLong(gcId));
		List<Goods> goodsList = this.goodsService
				.query("from Goods obj where obj.store_recommend = true and obj.gc.id = :gcId and obj.ad_acc.id != '' order by obj.store_recommend_time desc",
						map, -1, -1);
