...
public class GroupViewAction {
	...
	public ModelAndView group(HttpServletRequest request,
		...
		if ("".equals(type) || "goods".equals(type) || type == null) {
			...
			params.put("group_type", 0);//[0为商品团购，1为生活类团购，生活类需要发送消费码及商家验证消费码]
			params.put("status", 0);//[状态，0为正常，-1为关闭,-2为已经结束,1为即将开始] 
				...
				if (ga_id != null && !ga_id.equals("")) {
					...
					if (ids != null && ids.size() > 0) {
						...
						if (cga_id != null && !cga_id.equals("")) {
							paras.put("ids",
									this.genericIds(CommUtil.null2Long(cga_id)));
						} else {
							paras.put("ids", ids);
						}
	...
	public ModelAndView group_head(HttpServletRequest request,
		...
		List<GroupArea> gas = this.groupAreaService
				.query("select obj from GroupArea obj where obj.parent.id is null order by obj.ga_sequence asc",
						null, -1, -1);
		if (ga_id != null && !ga_id.equals("")) {
			GroupArea nga = this.groupAreaService.getObjById(CommUtil
					.null2Long(ga_id));
			if (nga != null) {
	...
	public ModelAndView group_view(HttpServletRequest request,
		...
		if (obj.getGroup().getBeginTime().before(new Date())//[ < true]
				&& obj.getGroup().getEndTime().after(new Date())//[ > true] 正在促销
				&& obj.getGg_status() == 1) {
			view = true;
		}
		if (user != null && user.getUserRole().indexOf("ADMIN") >= 0) {//[如果用户角色等于admin返回true]
			view = true;
		}
		if (view) {
	...
	public ModelAndView grouplife_search(HttpServletRequest request,
		...
		String path = System.getProperty("koalab2b2c.root") + File.separator
		...
		if (sort != null) {
			...
			pList = lucene.search(keyword, CommUtil.null2Int(currentPage),
					null, null, null, null, null, c_id, a_id, null);
		}
