...
import com.koala.module.app.service.IQRLoginService;
import com.koala.msg.MsgTools;
import com.koala.view.web.tools.ImageViewTools;
...
public class LoginViewAction {
	...
	public ModelAndView login(HttpServletRequest request,
		...
		boolean domain_error = CommUtil.null2Boolean(request.getSession(false).getAttribute("domain_error"));//如果不为空则为true
	...
	public String register_finish(HttpServletRequest request,
		...
		try {
			...
			if (reg) {
				...
				request.getSession(false).removeAttribute("verify_code");
				return "redirect:koala_login.htm?username="
	...
	private String generic_qr(HttpServletRequest request, String qr_session_id) {
		...
		String login_url = CommUtil.getURL(request)
				+ "/koala_login.htm?qr_session_id=" + rand_str;
		request.getSession().setAttribute("qr_session_id", rand_str);
		String logoPath = "";
