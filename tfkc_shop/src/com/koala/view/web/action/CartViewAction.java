...
import com.koala.core.tools.XMLUtil;
...
import com.koala.foundation.domain.query.DeliveryAddressQueryObject;
...
//						} else
	...
	public void goods_cart1_spec_save(HttpServletRequest request,
		...
		if (obj != null) {
			int goods_inventory = CommUtil.null2Int(this.generic_default_info(
					obj.getGoods(), gsp).get("count"));// 计算商品库存信息
			double price = CommUtil.null2Double(this.generic_default_info(
					obj.getGoods(), gsp).get("price"));// 计算商品库存信息
			if (goods_inventory == 0) {
				code = -100;
			} else {
			...
			if (order_carts.size() > 0 && addr != null) {
				...
				if (inventory_very) {
					...
					for (int i = 0; i < store_ids.length; i++) {// 根据店铺id，保存多个子订单
						...
						all_of_price = all_of_price + of
								.getTotalPrice().doubleValue();// 总订单价格
						if (sid.equals("self") || sid == "self") {
	...
	public ModelAndView order_pay(HttpServletRequest request,
		...
		if (order != null && order.getOrder_status() == 10) {
			...
			for (Goods obj : goods_list) {
				int order_goods_count = this.orderFormTools.queryOfGoodsCount(
						order_id, CommUtil.null2String(obj.getId()));
				String order_goods_gsp_ids = "";
				List<Map> goods_maps = this.orderFormTools.queryGoodsInfo(order
						.getGoods_info());
				for (Map obj_map : goods_maps) {
				...
				// 真实商品库存
				int real_goods_count = CommUtil.null2Int(this
						.generic_default_info(obj, order_goods_gsp_ids).get(
								"count"));// 计算商品库存信息
				if (order_goods_count > real_goods_count) {
					inventory_very = false;
					break;
						...
						for (Map map_combin : maps) {
							if (map_combin.get("combin_suit_info") != null) {
								Map suit_info = Json.fromJson(Map.class,
										CommUtil.null2String(map_combin
												.get("combin_suit_info")));
								...
								for (Map temp_goods : combin_goods) {// 更新组合套装中其他商品信息，将套装主商品排除，主商品在普通商品更新中更新信息
									for (Goods temp : goods_list) {
										if (!CommUtil.null2String(
												temp_goods.get("id")).equals(
												temp.getId().toString())) {
											Goods goods = this.goodsService
													.getObjById(CommUtil
															.null2Long(temp_goods
																	.get("id")));
						...
						for (Goods goods : goods_list) {
							...
							if (logspecmap.containsKey(spectype)) {
								logspecmap.put(spectype,
										logspecmap.get(spectype) + goods_count);
							} else {
									...
									if (Arrays.equals(temp_ids, gsp_list)) {
										temp.put(
												...
												CommUtil.null2Int(temp
														...
														- goods_count);
										if (CommUtil
												.null2Int(temp.get("count")) <= CommUtil
