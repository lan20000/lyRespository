...
import com.koala.foundation.service.IStoreSlideService;
	...
	public ModelAndView module_hotspot(HttpServletRequest request,
		...
		if (img != null && img.getWidth() > 680) {// 若图片宽度大于显示区域宽度，将所有热点区域按照比例放大显示
			...
			for (Map temp : temp_coors_list) {
				String temp_coor[] = CommUtil.null2String(temp.get("coor"))
						.split(",");
				String coor = "";
				for (String str : temp_coor) {
					if (str != "") {
						if (coor == "") {
							coor = CommUtil.null2String(Math.round(CommUtil
									.mul(rate, str)));
						} else {
							coor = coor + ","
									+ Math.round(CommUtil.mul(rate, str));
						}
					}
				}
				if (div != null && div.equals("div2")) {
					...
					String div2_coor[] = CommUtil.null2String(temp.get("coor"))
							.split(",");
					for (String str : div2_coor) {
						if (str != "") {
							if (new_coor == "") {
								new_coor = CommUtil.null2String(Math
										.round(CommUtil.mul(rate, str)));
							} else {
								new_coor = new_coor + ","
										+ Math.round(CommUtil.mul(rate, str));
							}
