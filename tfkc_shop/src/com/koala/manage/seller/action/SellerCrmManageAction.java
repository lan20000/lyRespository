...
import com.koala.foundation.domain.CustomerRelMana;
import com.koala.foundation.domain.Store;
import com.koala.foundation.domain.User;
import com.koala.foundation.domain.query.CustomerRelManaQueryObject;
import com.koala.foundation.service.IConsultService;
import com.koala.foundation.service.ICustomerRelManaService;
...
public class SellerCrmManageAction {
	...
	public ModelAndView crm_list(HttpServletRequest request,
		...
		CommUtil.saveIPageList2ModelAndView(url + "/seller/crm_list.htm", "",
				params, pList, mv);
		...
		if (message != null && !message.equals("")) {
			...
			for (String id : cids) {
				...
				if (crm != null
						...
						if(buyer!=null&&buyer.getEmail()!=null&&!buyer.getEmail().equals("")){
							if(count>0){
	...
	public void send_message_save(HttpServletRequest request,
			HttpServletResponse response, String ids, String message) throws UnsupportedEncodingException {
		...
		if (message != null && !message.equals("")) {
			...
			for (String id : cids) {
				...
				if (crm != null
						&& crm.getStore_id().equals(user.getStore().getId())) {
						User buyer = this.userService.getObjById(CommUtil.null2Long(crm.getUser_id()));
						if(buyer!=null&&buyer.getMobile()!=null&&!buyer.getMobile().equals("")){
							if(count>0){
		...
		for (String id : ids) {
			if (!id.equals("")) {
				...
				if (customerrelmana != null
						& customerrelmana.getStore_id().equals(
								user.getStore().getId())) {
