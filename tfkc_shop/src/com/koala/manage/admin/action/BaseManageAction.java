...
import com.koala.core.annotation.Log;
...
import com.koala.foundation.domain.LogType;
import com.koala.foundation.domain.StoreStat;
import com.koala.foundation.domain.SysConfig;
import com.koala.foundation.domain.SystemTip;
import com.koala.foundation.domain.User;
import com.koala.foundation.domain.query.SystemTipQueryObject;
...
import com.koala.foundation.service.IStoreStatService;
import com.koala.foundation.service.ISysConfigService;
import com.koala.foundation.service.ISystemTipService;
...
public class BaseManageAction {
	...
	public void login_success(HttpServletRequest request,
		...
		if (SecurityUserHolder.getCurrentUser() != null) {
			...
			HttpSession session = request.getSession(false);//得到session域对象  若存在会话则返回该会话，否则返回NULL
			...
			if (ajax_login) {
				...
				if("weixin".equalsIgnoreCase(login_type)){
					if(!"".equals(session.getAttribute("returnUrl")) && session.getAttribute("returnUrl") != null){
	...
	public ModelAndView login_error(HttpServletRequest request,
		...
		String koala_view_type = CommUtil.null2String(request.getSession(
				false).getAttribute("koala_view_type"));
		if (koala_view_type != null && !koala_view_type.equals("")) {
			if (koala_view_type.equals("weixin")) {
	...
	public ModelAndView sys_config_save(HttpServletRequest request,
			...
			String ios_download,String android_seller_download,String ios_seller_download,
			String app_seller_download) {
		...
		if (id.equals("")) {
			...
			sysConfig = (SysConfig) wf.toPo(request, obj);//[{codeStat=}, {id=1}, {poweredby=welcome-1}, {img0_text=}, {op_title=站点设置成功}, {title=Ebuyair.com}, {company_name=Ebuyair-1}, {img2_text=}, {sina_domain_code=}, {android_version=}, {sina_login_id=}, {android_download=}, {share_code=}, {sina_login=false}, {securityCodeRegister_ck=on}, {qq_login_id=}, {securityCodeType=normal}, {id=1}, {poweredby=welcome-1}, {img0_text=}, {op_title=站点设置成功}, {title=Ebuyair.com}, {company_name=Ebuyair-1}, {img2_text=}, {sina_domain_code=}]
		}
	...
	public ModelAndView authority(HttpServletRequest request,
		...
		if (domain_error) {
			mv = new JModelAndView("error.html", configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request,
					response);
			mv.addObject("op_title", "域名绑定错误，请与http://www.koala.com联系");
		}
		return mv;
	}
