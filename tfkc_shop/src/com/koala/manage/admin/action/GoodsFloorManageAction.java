...
import com.koala.foundation.domain.query.GoodsFloorQueryObject;
...
import com.koala.manage.admin.tools.GoodsFloorTools;
...
public class GoodsFloorManageAction {
	...
	public void restore_img(HttpServletRequest request,
		...
		Map map = new HashMap();
		GoodsFloor gf = this.goodsfloorService.getObjById(CommUtil
				.null2Long(id));
		if (gf.getIcon() != null) {
	...
	public ModelAndView goods_floor_style2(HttpServletRequest request,
		...
		if (CommUtil.null2Int(width) > 0 && CommUtil.null2Int(height) > 0) {
			...
			params.put("ap_type", "img");
			aps = this.advertPositionService
					.query("select obj from AdvertPosition obj where obj.ap_status=:ap_status and obj.ap_width=:ap_width and obj.ap_height=:ap_height and obj.ap_type=:ap_type order by obj.addTime desc",
							params, -1, -1);
		} else {
			params.put("ap_type", "img");
			params.put("ap_status", 1);
			aps = this.advertPositionService
					.query("select obj from AdvertPosition obj where obj.ap_status=:ap_status and obj.ap_type=:ap_type and obj.ap_status=:ap_status order by obj.addTime desc",
							params, -1, -1);
		}
		List<GoodsClass> gcs = this.goodsClassService
				.query("select obj from GoodsClass obj where obj.parent.id is null order by obj.sequence asc",
	...
	public String goods_floor_style2_save(HttpServletRequest request,
				...
				if (map.get("module_id").equals(module_id)) {
					...
					if (type.equals("adv")) {
						...
						String adv_url = CommUtil.getURL(request)
								+ "/advert_redirect.htm?url="
								+ ap.getAp_acc_url() + "&id=" + ap.getAdv_id();
						if (ap.getAp_show_type() == 0) {// 固定广告
							if (ap.getAdvs().size() > 0) {
								img_url = ap.getAdvs().get(0).getAd_acc()
										.getPath()
										+ "/"
			...
			if (!update) {
				...
				if (type.equals("adv")) {
					...
					String img_url = ap.getAp_acc().getPath() + "/"
							+ ap.getAp_acc().getName();
					String adv_url = CommUtil.getURL(request)
							+ "/advert_redirect.htm?url=" + ap.getAp_acc_url()
							+ "&id=" + ap.getAdv_id();
					if (ap.getAp_show_type() == 0) {// 固定广告
						if (ap.getAdvs().size() > 0) {
