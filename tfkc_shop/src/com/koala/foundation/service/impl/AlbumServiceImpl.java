...
public class AlbumServiceImpl implements IAlbumService {
	...
	public IPageList list(IQueryObject properties) {
		...
		GenericPageList pList = new GenericPageList(Album.class, construct,
				query, params, this.albumDao);
		if (properties != null) {
	...
	public Album getDefaultAlbum(Long id) {
		...
		if (user.getParent() == null) {
			...
			List<Album> list = this.albumDao
					.query("select obj from Album obj where obj.user.id=:user_id and obj.album_default=:album_default",
							params, -1, -1);
			if (list.size() > 0)
				return list.get(0);
			else
